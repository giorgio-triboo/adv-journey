...
2. Sostituisci `deploy/nginx/nginx.conf` con una config che ascolta 443 ssl e monta `/etc/nginx/ssl` (aggiungi il volume `./deploy/ssl:/etc/nginx/ssl:ro` al servizio nginx in `docker-compose.prod.yml`).
