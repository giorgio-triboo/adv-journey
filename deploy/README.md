# Deploy cepu-lavorazioni su server con Docker
...
- File `.env` nella root del progetto (creare da `.env.example`)

### Porte

- **80**: nginx (app principale)
- **18080**: Adminer (gestione DB)
...
- **nginx** sulla porta 80: reverse proxy verso l’istanza attiva.
...
```bash
cd /home/ec2-user/cepu-lavorazioni
# .env già configurato
...
2. **Copia file**: artefatto in `/home/ec2-user/cepu-lavorazioni`. `.env` non va nel repo.
3. **AfterInstall**: `afterinstall.sh` fa build, `up -d`, migrazioni Alembic.

- **appspec.yml**: destinazione `/home/ec2-user/cepu-lavorazioni`
